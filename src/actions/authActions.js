    ...
    return (dispatch) => {
        ...
        if (token) {
            apiPromise({token}, 'auth/loginWithToken').then(
                (data) => {
                    dispatch({type: types.LOGIN_SUCCESS, coins: data.coins, name: data.username, token: data.token})
                },
                (err) => {
    ...
    return (dispatch) => {
        ...
        apiPromise(options, 'auth/login').then(
            (data) => {
                ...
                return dispatch({type: types.LOGIN_SUCCESS, coins: data.coins, name: username, token: data.token});
            }, (err) => {
                if (err === 'unauthorized') {
...
const resetPw = (options, tokenUrl, dispatch) => {
    ...
    return (dispatch) => {
        apiPromise({username, password}, `auth/reset/${tokenUrl}`).then(
            (data) => dispatch({type: types.LOGIN_SUCCESS, coins: data.coins, name: data.username, token: data.token}),
...
const signup = (credentials, dispatch) => {
    return (dispatch) => {
        ...
        apiPromise(options, 'auth/signup').then(
            (data) => {
                if (saveData) window.localStorage.setItem('token', data.token);
                return dispatch({type: types.SIGNUP_SUCCESS, coins: 0, name: username, token: data.token});
            }, (err) => {
                if (err === 'unauthorized') {
